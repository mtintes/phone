#!/usr/bin/env python
#CREDIT: https://markhedleyjones.com/projects/python-tone-generator
...
class ToneGenerator(object):
    ...
    def sinewave(self):
        if self.buffer_offset + self.frames_per_buffer - 1 > self.x_max:
            ...
            xs = numpy.arange(self.buffer_offset,
                              self.x_max)
            tmp = self.amplitude * (numpy.sin(xs * self.omega) + numpy.sin(xs * self.omega))
            out = numpy.append(tmp,
                               numpy.zeros(self.frames_per_buffer - len(tmp)))
        else:
            xs = numpy.arange(self.buffer_offset,
                              self.buffer_offset + self.frames_per_buffer)
            out = self.amplitude * (numpy.sin(xs * self.omega) + numpy.sin(xs * self.omega2))
        self.buffer_offset += self.frames_per_buffer
        return out
    ...
    def play(self, frequency, duration, amplitude):
        self.omega = float(350) * (math.pi * 2) / self.samplerate
        self.omega2 = float(440) * (math.pi * 2) / self.samplerate
...
# frequency_start = 350        # Frequency to start the sweep from
# frequency_end = 350       # Frequency to end the sweep at
# num_frequencies = 1       # Number of frequencies in the sweep
amplitude = 0.50            # Amplitude of the waveform
# step_duration = 100.00        # Time (seconds) to play at each step
...
# stream = generator.play(frequency_start, step_duration, amplitude)
